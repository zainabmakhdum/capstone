...
According to *[The Rape, Abuse, and Incest National Network](https://www.rainn.org/statistics/scope-problem)* (RAINN), every 68 seconds, an American falls victim to sexual assault. Moreover, one in six American women and one in thirty-three American men experience either attempted or completed sexual assault in their lifetime.

While substantial progress has been made to eliminate the stigmas surrounding this issue, it is important to keep the momentum going on sexual assault awareness in order to combat it effectively. Therefore, the purpose of this research is to determine the following: how do demographic, geographical, and relational factors influence the varying patterns of rape incidents across the United States in 2022? 
...
For this project, the data was obtained from the *Federal Bureau of Investigation* (FBI) through their [Crime Explorer Online Tool](https://cde.ucr.cjis.gov/LATEST/webapp/#/pages/home). The FBI utilizes the Uniform Crime Reporting (UCR) Program to collect data from over 18,000 city, county, state, university and college, tribal, and law enforcement agencies. Due to discrepancies in the legal definition of rape, sexual assault, and sexual abuse on a state by state case, it is important to note that certain limitations exist in the data collection for some states. 

Approximately nineteen datasets were extracted via the Python programming language from this database which were comprised of crucial information on the victims by state, the precise location of a sex offense, victim-offender relationship, victim and arrestee demographics (age, race, and sex), etc.

# Data Preprocessing

Each of the nineteen datasets consisted of information on a separate aspect of all criminal activity – including violent and property crime. For instance, one file consisted of crimes categorized by state, while another file consisted of crimes classified by county. Since this research primarily focuses on examining sex offenses, which are categorized under violent crimes, the initial step was to extract data specifically related to this form of criminal activity.

Other steps in data preparation included imputing the missing values based on the median and mode, depending on whether the variable was quantitative or qualitative, as well as changing datatypes and making the column headers easily interpretable and accessible. In the case of offenses by state and county datasets, importing the files yielded missing values due to Excel file formatting issues. Therefore, the best course of action was to replace the missing values based on the nearest non-missing value. Moreover, *Federal Information Processing System* (FIPS) codes were utilized to ensure that the county and state names matched for all observations.

To prepare the data for modeling, multiple datasets were merged to develop a comprehensive dataset of sex offenders in NY state for 2022. This incorporated relevant features such as age, sex, location of crime, and weapons used during the crime. Due to the presence of categorical variables, dummy variables we created, and the issue of class imbalance was tackled using the Synthetic Minority Over-sampling Technique (SMOTE). Furthermore, the data was standardized and a check for multicollinearity resulted in removing a highly correlated variable. Lastly, the data was split into training, testing, and validation sets based on a 5:1:1 ratio. 
...
The analysis of the varying patterns in sex offenses is divided into several distinct parts. Below are some of the aspects this study aims to decipher throughout each section:

- **Overview of Sex Offense Dynamics** (@sec-overview)**:** Are there any disparities observed between the total number of victims compared to the total number of individuals arrested?
- **Geographic Analysis** (@sec-geoanalysis)**:** How do rape incidents fluctuate across different regions - including states and counties - in the United States?
- **Temporal Analysis** (@sec-tempanalysis)**:** What causes deviations in rape rates over time (2012-2022) on a national level?
- **Demographic Analysis** (@sec-demoganalysis)**:** Does the demographic information (age, race, and sex) vary for victims vs. arrestees?
- **Relational Analysis** (@sec-relanalysis)**:** Do relationships exist between victims and perpetrators of sex offenses?
- **Case Study** (@sec-casestudy)**:** Do seasonal trends in rape incidents exist across NY state in 2022?

It should be emphasized that this analysis is conducted while keeping in mind the discrepancies in population densities, youth concentration, economic conditions, cultural factors, the effectiveness of law enforcement agencies, crime reporting practices, etc., as these factors vary from place to place.

## Overview: Sex Offense Dynamics {#sec-overview}

```{python}
#| label: Fig-1
#| fig-cap: "Victims vs. Arrests & Completed vs. Arrested Statistics (2022)"
#| warning: false
...
## Geographic Analysis {#sec-geoanalysis}

The first stage of the geographic analysis includes the examination of regional trends by observing how rape incidents fluctuate from state to state. 
...
```{python}
#| label: Fig-2
#| fig-cap: "Sex Offenses Per Capita by State for 2022"
#| warning: false
...
Based on @Fig-2 above, regional discrepancies are displayed as western states such as Alaska, Montana, and Utah had the highest sex offenses per capita, while southern states such as Florida and Alabama had the lowest sex offenses per capita. It can also be noted that except for New Hampshire, the East Coast had relatively fewer sex offenses per capita in comparison to states across the West Coast. 

Next, to analyze the distribution of sex offenses within states, @Fig-3 shows reported sex offenses by counties. It is important to note that the data points do not reflect county totals but are the number of offenses reported by the sheriff's office or county police department.
...
Due to the majority of counties having relatively low reported sex offense cases, this map incorporated a logarithmic scale where the actual figures of sex offenses were compressed between 0.0 and 3.0. 
:::

```{python}
#| label: Fig-3
#| fig-cap: "Sex Offenses by U.S. Counties for 2022"
#| warning: false
...
# source for editing colorbar: https://plotly.com/python/reference/layout/coloraxis/
...
From @Fig-4, it can be deduced that the year 2012 had the lowest number of rape incidents whereas 2016 had the highest number of rape incidents. This nationwide trend shows rape incidents rapidly increasing by 129.97% from approximately 85,141 rape cases in 2012 to 195,804 rape cases in 2013. This significant increase was due to a change in the legal definition of rape by the FBI in 2013 – as described in the Legal Definitions (@tip-legal-definitions) section. 

The figures steadily rose until 2016 (229,384 rape cases), during which rape incidents reached an all-time high between 2012-2022. There was a sharp decline from 2016 to 2017 by approximately 40.86%, after which the rape incidents fluctuated. For the year with the most recent data available, i.e., 2022 (133,294 rape cases), it is observed that although the number of rape incidents was still higher than ten years ago in 2012, they still, however, remain lower than they were in 2016.  

It is important to acknowledge that the fluctuations of rape incidents from 2016-2017 may be attributed to the fact that this was a tumultuous time for women's rights movements. For instance, the #MeToo movement united women across the nation due to reports of sexual abuse by Hollywood producer [Harvey Weinstein](https://abcnews.go.com/US/timeline-harvey-weinsteins-path-york-city-rape-sexual/story?id=67708458) and other powerful individuals. Moreover, the highly publicized case of [People v. Turner](https://harvardlawreview.org/print/vol-132/california-judge-recalled-for-sentence-in-sexual-assault-case/), brought national attention and outrage towards the cracks within the U.S. justice system regarding punishing the perpetrators. 

These social movements and high-profile court cases have the ability to implement societal changes and at the very least, bring awareness towards an issue, and the fluctuations of rape incidents in 2016-2017 serve as an indicator regarding a shift in public discourse surrounding sexual violence. 

## Demographic Analysis {#sec-demoganalysis}
...
## Relational Analysis {#sec-relanalysis}
...
```{python}
#| label: Fig-7
#| fig-cap: "Relationship Between Victims and Offenders (2022)"
#| warning: false
...
```{python}
#| label: Fig-8
#| fig-cap: "Sex Offenses By Location (2022)"
#| warning: false
...
@Fig-7 and @Fig-8 show that 79.88% of the sex offenders were either a family member of the victim or were known to the victim, while only 7.16% were strangers. This figure coincided with the observation that sex offenses commonly occurred at the residence/home. 

Moreover, @Fig-8 indicates that one of the locations where sexual offenses commonly take place is in secondary or elementary school. This supports the data from @Fig-5 which indicates that most sex offense victims belonged to the younger age categories. 

## Case Study: Seasonal Trends In Sex Offenses For New York {#sec-casestudy}
...
Furthermore, the highest number of sex offenses took place on Saturdays – approximately 306 – while the lowest number of sex offenses took place on Sundays – approximately 259. This indicates a relatively minor disparity in sex offense rates. In fact, the *[National Institute of Justice](https://nij.ojp.gov/topics/articles/factors-increase-sexual-assault-risk)* (NIJ) claims that “More than half of the sexual assaults took place on weekends.”

# Modelling: Completed vs. Attempted Sex Offenses

Supervised learning algorithms were utilized to examine the binary classification problem of determining what factors (age, sex, location, and weapons) influenced whether a sex offense was completed or attempted in New York. Classification models such as logistic regression, decision trees, random forests, and support vector machine (SVM) were trained, tested, and finetuned to reduce overfitting. 
...
top_5_df_df = pd.DataFrame({'Feature': top_5_dt.index, 'Importance Score': top_5_dt.values.round(3)})
...
From the feature importance scores for the random forest model in @Tbl-2, it can be deduced that the age of an offender, the sex of an offender being male, and unknown place and home/residence being the location of the crime, plays a significant role in determining the outcome of a sex offense i.e., whether it was completed or attempted.

Both the decision tree and random forest models showing approximately the same features indicate that these features are most influential in predicting the outcome.
...
print("===================================")
print("Accuracy:", round(accuracy_score(y_valid, y_pred_valid_ensemble), 4))
print("Precision:", round(precision_score(y_valid, y_pred_valid_ensemble), 4))
print("Recall:", round(recall_score(y_valid, y_pred_valid_ensemble), 4))
print("F1 Score:", round(f1_score(y_valid, y_pred_valid_ensemble), 4))
print("ROC AUC Score:", round(roc_auc_score(y_valid, y_pred_valid_ensemble), 4 ))
print("===================================")
# END citation
# ---------------------------------------------------------
```
...
On the other hand, modeling efforts revealed some significant features that influenced the outcome of whether a sex offense was completed or attempted. Lastly, while geographic analysis uncovered certain regional trends, however, it was inconclusive in the study to determine why some areas had high sex offenses while others had low sex offenses.

Consequently, future research may include examining cultural and societal influences on sex crime reporting rates. All in all, it is crucial to foster a safe environment for victims through developing new and improving old support mechanisms, and encouraging victims to speak out against the perpetrators.
